...
export default function Dashboard() {
  ...
  const handleApplyPatch = async () => {
    ...
    addLog(`✅ PARCHE: ${selectedAlert.cweId} mitigada. Red Team bloqueado.`, 'patch');
    setPatchingStatus('');
    setPatchInput('');
    setSelectedAlert(null);
    setAiTranslation('');
  ...
  if (!roeAccepted) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-lg bg-slate-900/80 backdrop-blur border border-red-500/40 rounded-2xl p-10 shadow-2xl shadow-red-900/20 text-slate-200">
          ...
          <p className="text-xs text-center text-red-400 uppercase tracking-widest mb-6">Blue Team Defense Center — Acceso Restringido</p>
          <div className="bg-slate-950 border border-slate-800 rounded-lg p-4 mb-6 text-sm text-slate-400 space-y-2">
            <p>• Entorno restringido a personal autorizado del <span className="font-bold text-blue-400">Blue Team</span>.</p>
          ...
          <button onClick={() => { setRoeAccepted(true); addLog('RoE aceptadas. Módulos habilitados.', 'info'); }}
            ...
            <CheckCircle className="w-5 h-5" /> Acepto — Ingresar al Centro de Defensa
          </button>
        </div>
      </div>
    );
