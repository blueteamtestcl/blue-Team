...
export function generateTechnicalReport(data: ReportData): void {
  ...
  doc.text('TECHNICAL PLAYBOOK:', 105, 125, { align: 'center' });
  doc.text('GUÍA DE REMEDIACIÓN', 105, 137, { align: 'center' });

  // Subtítulo
  doc.setFontSize(12);
  doc.setTextColor(...COLORS.accent);
  doc.text('INTERNAL BLUE TEAM DOCUMENTATION', 105, 152, { align: 'center' });
...
${crit > 0 ? 'ADVERTENCIA: Se detectaron vulnerabilidades críticas que requieren atención inmediata para prevenir compromiso total del sistema.' : 'No se detectaron vulnerabilidades de nivel crítico.'}`;
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('4. Detalles de Vulnerabilidades y Mitigación Técnica', margin, y);
  y += 12;
  ...
  data.alerts.forEach((alert, index) => {
    ...
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(`Hallazgo #${index + 1}: ${alert.cweId} - ${alert.name}`, margin + 2, y + 6);
    y += 12;

    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(50, 50, 50);
    doc.text(doc.splitTextToSize(`Descripción: ${alert.description}`, contentW), margin, y);
    y += 15;
  ...
  // Reemplazamos la renderizacion tabular con la guia de la IA arriba
